...
public class MyLinkedList05<E> {
	...
	public void add(int index,E element){//alt+shift+R重构
		checkRange(index);
		Node newNode = new Node(element);
		Node temp = getNode(index);
		
		if(temp!=null){
			Node up = temp.previous;//原关系
			up.next = newNode;//加入新节点
			
			newNode.next = temp;//建立新关系
			temp.previous = newNode;//建立新关系
		}
	}
	...
	public E get(int index){//因为不是数组，所以get方法和之前的实现不同
		checkRange(index);
		Node temp = getNode(index);//获取node节点
		return temp!=null?(E)temp.element:null;//免得报空指针
	}
